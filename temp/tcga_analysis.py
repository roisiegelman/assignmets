...
def search_tcga_files(project_id, data_type="gene_expression_quantification"):
    ...
    print("Raw API response for file search:", response.json())
...
if __name__ == '__main__':
    ...
    print(tcga_files.columns)  # Print columns to debug missing 'file_id' issue
    ...
    if not tcga_files.empty:
        for index, row in tcga_files.iterrows():
            file_id = row['file_id']
            file_name = row.get('file_name', file_id)  # Use file_id if file_name is not available
        ...
        print("No files found for the given search criteria.")
